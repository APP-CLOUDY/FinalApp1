...
final class NewRewardViewController: UIViewController {
    ...
    private func displayTitle(for subType: String) -> String {
        switch subType {
        case "gadget_time": return "Gadget Time"
        case "tv_time": return "TV Time"
    ...
    private let rewardTypeRow = RewardSelectRow(title: "Reward Type*")
    ...
    private func openRewardTypeSelector() {
        ...
        vc.onSelect = { [weak self] selectedKey in
            guard let self = self else { return }

            self.rewardTypeRow.setDetail(
                self.displayTitle(for: selectedKey), // UI text
                value: selectedKey                  // backend value
            )
        }
