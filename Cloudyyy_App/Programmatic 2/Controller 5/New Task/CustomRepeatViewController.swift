...
//  Cloudyyy_App
//
//  Created by user@10 on 24/12/25.
//
...
class CustomRepeatViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        // Default to current day for weekly if nothing selected
        if selectedWeekdays.isEmpty {
            let currentDay = Calendar.current.component(.weekday, from: Date())
            selectedWeekdays.insert(currentDay)
        }
    }
    ...
    private func setupWeekdays() {
        ...
        for (index, title) in days.enumerated() {
            ...
            btn.layer.cornerRadius = 18 // Circular
            btn.tag = index + 1 // Calendar weekday 1-7
