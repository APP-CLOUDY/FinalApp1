...
final class TaskFormViewController: UIViewController {
    ...
    // New: Interval (e.g., "Every 2 Weeks")
    ...
    // New: Specific Days (Only for Weekly)
    ...
    private func applyDefaults() {
        ...
        intervalRow.setDetail("Every 1")
        endRepeatRow.setDetail("Never")
        daysRow.setDetail("Select Days")
    }
    ...
    private func populate(_ task: ScheduleTaskModel) {
        ...
        // Note: frequencyRow detail will be set by updateDynamicFrequencyLabel() at end
        ...
        // ✅ Updates UI visibility and generates the smart label
        updateRepeatVisibility()
        updateDynamicFrequencyLabel()
    }
        ...
        Task {
            do {
                ...
                await MainActor.run {
                    ...
                    if case .edit(let task) = self.mode {
    ...
    private func setupStaticMenus() {
        ...
        // ✅ Updated Frequency Menu (includes Custom)
        ...
        let intervalActions = (1...30).map { i in
            UIAction(title: "\(i)") { [weak self] _ in
                ...
                self?.updateDynamicFrequencyLabel() // ✅ Call Smart Label
            }
        }
        intervalRow.setMenu(UIMenu(children: intervalActions))
    }
    
    // ✅ NEW Helper: Updates state and triggers Smart Label
    ...
    // ✅ NEW Helper: Generates "Every 2 Weeks on Mon, Fri"
    ...
    private func openCustomRepeat() {
        ...
        // Populate existing data into the custom screen if needed (optional improvement)
        // For now, we capture result:
    ...
    private func openDaysMenu() {
        ...
        for (id, name) in days {
            ...
            alert.addAction(UIAlertAction(title: title, style: .default, handler: { [weak self] _ in
                ...
                self?.updateDaysLabel()
                self?.updateDynamicFrequencyLabel() // ✅ Update Smart Label
                self?.openDaysMenu()
            }))
        }
    ...
    @objc private func endDatePickerChanged() {
        ...
        updateDynamicFrequencyLabel() // Update if needed
    }
