//
//  QuickRewardsViewController.swift
//  Cloudyyy_App
//
//  Same layout as Dream It / Spring On
//  Uses ChildManager.shared.quickRewards(for:)
//
...
final class QuickRewardsViewController: UIViewController {
    ...
    private func reloadForKid(_ kid: Kid) {
        activeItems = ChildManager.shared.quickRewards(for: kid.id) ?? []
        completedItems = activeItems.filter { !$0.isActive }

        let activeOnly = activeItems.filter { $0.isActive }

        populateActive(activeOnly)
    ...
    private func populateCompleted(_ list: [RewardDetailItem]) {
    ...
    private func showKidsMenu() {
        ...
        menu.onKidSelected = { k in ChildManager.shared.selectedKid = k }
